...
import { Plus, Pencil, Trash2, Tags, Image as ImageIcon, Loader2 } from "lucide-react";
...
export default function CategoriesPage() {
  ...
  const openDialog = (item?: Category) => {
    if (item) {
      setEditItem(item);
      setForm({ name: item.name, slug: item.slug, description: item.description ?? "", image_url: item.image_url ?? "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { name: form.name, slug: form.slug, description: form.description, image_url: imageUrl };
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || `Failed to ${editItem ? 'update' : 'create'} category`);
      }

      toast.success(editItem ? "Category updated!" : "Category created!");
      setDialogOpen(false);
      load(); // Reload categories after successful operation
    } catch (error: any) {
    ...
    <div>
      ...
      {dialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setDialogOpen(false)} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">Category Image</label>
                {imagePreview && (
