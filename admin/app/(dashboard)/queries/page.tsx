...
export default function QueriesPage() {
  ...
  const loadQueries = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Failed to load queries", { description: error.message });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const markAsRead = async (id: string, currentStatus: boolean | null) => {
    ...
    try {
      ...
      setQueries(queries.map(q => q.id === id ? { ...q, is_read: !currentStatus } : q));
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (viewQuery?.id === id) setViewQuery(null);
    } catch (error: any) {
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Customer messages from the contact form</p>
        </div>
      </div>
                  ...
                  <td className="px-4 sm:px-6 py-4">
                    {q.is_read ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-orange-50 text-orange-600 text-xs font-medium rounded-full">
                        <span className="w-1.5 h-1.5 bg-orange-400 rounded-full" /> Unread
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-4 sm:px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {!q.is_read && (
                        ...
                        >
                          {processingId === q.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                        </button>
                      )}
                      <button
      ...
      {viewQuery && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={() => setViewQuery(null)} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10">
            <div className="flex items-start justify-between mb-5">
              <div>
                <h2 className="font-bold text-gray-900 text-lg">{viewQuery.name}</h2>
                <p className="text-sm text-gray-400">{viewQuery.email}</p>
                {viewQuery.phone && <p className="text-sm text-gray-400">{viewQuery.phone}</p>}
              </div>
              <button onClick={() => setViewQuery(null)} className="p-2 rounded-lg hover:bg-gray-100 transition text-gray-400">✕</button>
            </div>
            {viewQuery.subject && (
              <div className="mb-3">
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Subject</span>
                <p className="text-sm text-gray-700 mt-1">{viewQuery.subject}</p>
              </div>
            )}
            <div>
            ...
            <p className="text-xs text-gray-400 mt-4">{formatDate(viewQuery.created_at)}</p>
            <button
              onClick={() => { handleDelete(viewQuery.id); setViewQuery(null); }}
